...
class orderController extends Controller
{
    ...
    {
        $orders = orderItem::where('status', '>', 7)->join('orders', 'orders.id', '=', 'order_items.order_id')->where('order_items.user_id', auth()->user()->id)->where('order_items.verified', 1)->orderBy('order_items.id', 'desc');
    ...
    {
        $orders = orderItem::where('order_items.verified', 1)->join('orders', 'orders.id', '=', 'order_items.order_id')->where('order_items.user_id', auth()->user()->id);
        if ($request->has('start')) {
            $startTime = CalendarUtils::createCarbonFromFormat('Y/m/d', $request->start)->toDateTimeString();
            $orders = $orders->where('orders.created_at', '>=', $startTime);
        }
